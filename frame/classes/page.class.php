<?php
	class Page {
		...
		private function firstprev(){
			if($this->page > 1) {
				$str="&nbsp;<a href='{$this->uri}page/1' target=\"_self\">{$this->config["first"]}</a>";
				$str.="&nbsp;<a href='{$this->uri}page/".($this->page-1)."' target=\"_self\">{$this->config["prev"]}</a>&nbsp;";		
				return $str;
			}
		...
		private function pageList(){
			...
			for($i=$inum; $i>=1; $i--){
				...
				if($page>=1)
					$linkPage.="<a href='{$this->uri}page/{$page}' target=\"_self\">{$page}</a>&nbsp;";
		...
		private function nextlast(){
			if($this->page != $this->pageNum) {
				$str='&nbsp;<a href="'.($this->uri).'page/'.($this->page+1).'" target="_self">'.($this->config["next"]).'</a>&nbsp;';
				$str.='&nbsp;<a href="'.($this->uri).'page/'.($this->pageNum).'" target="_self">'.($this->config["last"]).'</a>&nbsp;';
				return $str;
			}
